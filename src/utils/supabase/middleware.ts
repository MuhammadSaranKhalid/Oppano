...
//   console.log("Organization Membership: ", orgMembership);
...
//       if (newOrganization) {
//         organizationId = newOrganization.id;
//         userRole = "ADMIN"; // New organization: assign admin role.
//       }
//     } else {
//       organizationId = organization.id;
//       userRole = "MEMBER"; // Existing organization: assign member role.
//     }

//     // Insert the organization membership if an organization ID was determined.
//     if (organizationId) {
//       const { error: membershipError } = await supabase.from("OrganizationUser").insert([
//         {
//           organizationId,
//           userId: user.id,
//           role: userRole,
...
//   // Ensure the organization has a "General" channel and add the user to it.
//   if (organizationId) {
//     // Look for an existing "General" channel.
...
//     if (generalChannel) {
//       // Add the current user as a participant in the "General" channel.
...
//     if (userRole === "ADMIN") {
//       // For an admin, create a self-chat conversation.
//       const { data: selfChat, error: selfChatError } = await supabase
//         .from("Conversation")
//         .insert({
//           organizationId,
//           type: "PRIVATE",
...
//       // For a member, create a private conversation with an admin.
...
//         const { data: privateConv, error: pcError } = await supabase
//           .from("Conversation")
//           .insert({
//             organizationId,
//             type: "PRIVATE",
...
//         if (pcError) {
//           console.error("Error creating private conversation:", pcError);
//         } else if (privateConv) {
//           await supabase.from("ConversationParticipant").insert([
//             {
